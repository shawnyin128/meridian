...
class LabGraphTests(unittest.TestCase):
    ...
    def test_lab_skill_documents_generated_graph_boundary(self) -> None:
        ...
        for skill_path in skill_paths:
            with self.subTest(skill=skill_path):
                ...
                self.assertIn("passive, read-only visibility into Agent work", skill_text)
    ...
    def test_apply_update_creates_confirmed_child_node_and_branch_edge(self) -> None:
        with TemporaryDirectory() as tmp:
            root = Path(tmp)
            self._write_minimal_lab(root)
            from meridian.lab.graph import apply_lab_update
            ...
            packet = {
                "schema": "meridian.lab.update.v1",
                "intent": "create_repair_branch",
                "target_thread": "kv-compression",
                "changes": [
                    {
                        "op": "create_node",
                        "node_id": "kv-compression.B",
                        "title": "Amortized scoring repair",
                        "parent": "kv-compression.A",
                        "state": "unresolved",
                        "why": "The baseline exposes a scoring bottleneck.",
            ...
            self.assertIn("### Node B: Amortized scoring repair", thread)
            self.assertIn("- parent: kv-compression.A", thread)
            self.assertIn("#### Why", thread)
            graph = json.loads((root / ".meridian/graph/graph.json").read_text(encoding="utf-8"))
            self.assertIn("kv-compression.B", {node["id"] for node in graph["nodes"]})
            self.assertIn(
    ...
    def test_apply_update_rejects_create_node_without_confirmation_or_title(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            result = apply_lab_update(
                ...
                {
                    "schema": "meridian.lab.update.v1",
                    "intent": "unconfirmed_branch",
                    "target_thread": "kv-compression",
                    "changes": [{"op": "create_node", "node_id": "kv-compression.B"}],
                    "user_confirmation": {"required_for": ["create_node"], "status": "missing"},
                },
            )

            self.assertEqual(result["status"], "rejected")
            self.assertEqual(thread.read_bytes(), before)
            codes = {finding["code"] for finding in result["validation"]["findings"]}
            self.assertIn("confirmation_required", codes)
            self.assertIn("invalid_create_node_title", codes)
    ...
    def test_apply_update_rejects_valid_but_unsupported_apply_op_without_writing(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            packet = {
                "schema": "meridian.lab.update.v1",
                "intent": "unsupported_update_edge_apply",
                "target_thread": "kv-compression",
                "changes": [
                    {
                        "op": "update_edge",
    ...
    def test_apply_update_writes_create_edge_once_into_graph_relations(self) -> None:
        with TemporaryDirectory() as tmp:
            root = Path(tmp)
            self._write_minimal_lab(root)
            thread = root / ".meridian/threads/kv-compression.md"
            ...
            first = apply_lab_update(root, packet)
            once = thread.read_text(encoding="utf-8")
            second = apply_lab_update(root, packet)

            self.assertEqual(first["status"], "applied")
            self.assertIn(".meridian/threads/kv-compression.md", first["written_paths"])
            self.assertEqual(second["status"], "applied")
            self.assertEqual(thread.read_text(encoding="utf-8"), once)
            self.assertEqual(once.count("- `kv-compression.A` related_to `kv-compression.B`"), 1)
            graph = json.loads((root / ".meridian/graph/graph.json").read_text(encoding="utf-8"))
            self.assertIn(
                ("kv-compression.A", "kv-compression.B", "related_to"),
    ...
    def test_apply_update_writes_doing_and_why_fields(self) -> None:
        with TemporaryDirectory() as tmp:
            root = Path(tmp)
            self._write_minimal_lab(root)
            from meridian.lab.graph import apply_lab_update
                    ...
                    "changes": [
                        {
                            "op": "update_node",
                            "node_id": "kv-compression.A",
                            "fields": {"doing": "Run the eviction probe.", "why": "Baseline is too slow."},
                        }
                    ],
                    "user_confirmation": {"required_for": [], "status": "not_required"},
                },
            ...
            self.assertEqual(result["status"], "applied")
            graph = json.loads((root / ".meridian/graph/graph.json").read_text(encoding="utf-8"))
            markdown = next(node for node in graph["nodes"] if node["id"] == "kv-compression.A")["markdown"]
            self.assertIn("#### Doing\n\nRun the eviction probe.", markdown)
            self.assertIn("#### Why\n\nBaseline is too slow.", markdown)
            self.assertEqual(graph["node_details"]["kv-compression.A"]["next_action"], "Define the first probe.")
