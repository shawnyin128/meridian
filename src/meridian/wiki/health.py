...
<head>
  ...
  <style>
    ...
    * {{ box-sizing: border-box; }}
    ...
    .actions {{ display:flex; gap:8px; justify-content:flex-end; align-items:center; margin-top:12px; }}
    .actions button {{ border:1px solid #b8b5aa; background:#fff; color:var(--ink); border-radius:7px; padding:7px 10px; font:inherit; font-weight:650; cursor:pointer; }}
    .actions button:disabled {{ cursor:not-allowed; opacity:.6; }}
    .run-status {{ min-height:18px; margin-top:8px; font-size:12px; color:var(--muted); }}
...
<body>
  <main>
    <header>
      ...
      <div class="score">
        ...
        <div class="actions">
          <button id="run-health" type="button">Run Check</button>
        </div>
        <div id="run-status" class="run-status">Start <code>meridian wiki health-ui --wiki-root wiki</code> to enable the button.</div>
      </div>
    </header>
  ...
  <script>
    (() => {{
      const button = document.getElementById("run-health");
      const status = document.getElementById("run-status");
      const endpoint = "http://127.0.0.1:8765/api/health/run";
      let running = false;
      async function runHealth() {{
        if (running) return;
        running = true;
        button.disabled = true;
        status.textContent = "Running health check...";
        try {{
          const response = await fetch(endpoint, {{ method: "POST" }});
          const payload = await response.json().catch(() => ({{}}));
          if (response.status === 409) {{
            status.textContent = payload.message || "A health check is already running.";
            return;
          }}
          if (!response.ok) {{
            status.textContent = payload.message || "Health check failed.";
            return;
          }}
          status.textContent = `Completed: ${{payload.health_level || "unknown"}} · ${{payload.overall_score ?? "?"}}. Opening updated report...`;
          setTimeout(() => {{ window.location.href = "http://127.0.0.1:8765/wiki-health.html"; }}, 900);
        }} catch (error) {{
          status.textContent = "Local health bridge is not running.";
        }} finally {{
          running = false;
          button.disabled = false;
        }}
      }}
      button.addEventListener("click", runHealth);
    }})();
  </script>
</body>
</html>
