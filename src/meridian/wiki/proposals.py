...
def _sources_from_context(context: dict[str, Any], *, wiki_root: Path) -> list[dict[str, Any]]:
    ...
    for result in context.get("results") or []:
        ...
        trust = provenance.get("trust") if isinstance(provenance.get("trust"), dict) else {}
        relative_path = str(
        ...
        quality_gate = result.get("quality_gate") or trust.get("quality_gate") or page_frontmatter.get("quality_gate")
        review_state = result.get("review_state") or trust.get("review_state") or page_frontmatter.get("review_state")
        confidence = result.get("confidence") or trust.get("confidence") or page_frontmatter.get("confidence")
        ...
        sources.append(
            {
                ...
                "page_id": result.get("page_id") or str(result.get("id") or "").removeprefix("wiki:")
                or Path(relative_path).with_suffix("").as_posix(),
                ...
                "matched_sections": [
                    ...
                    for item in result.get("matched_sections") or result.get("sections") or []
                ],
