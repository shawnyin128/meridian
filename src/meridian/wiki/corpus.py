...
def _coerce_frontmatter_scalars(value: Any) -> Any:
    """Restore Meridian's scalar semantics after BaseLoader builds the YAML tree."""
    if isinstance(value, dict):
        return {str(key): _coerce_frontmatter_scalars(item) for key, item in value.items()}
    if isinstance(value, list):
        return [_coerce_frontmatter_scalars(item) for item in value]
...
def parse_frontmatter(text: str) -> dict[str, Any]:
    ...
    if not isinstance(loaded, dict):
        return {}
    return _coerce_frontmatter_scalars(loaded)
