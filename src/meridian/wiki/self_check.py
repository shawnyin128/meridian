...
def build_quality_agent_packet(*, run_manifest: Path, out_path: Path) -> Path:
    ...
    packet = [
        ...
        "- A researcher wants to cite the paper but must first inspect limitations, calibration dependence, model-family restrictions, and claims that should not generalize.",
        "- A reader needs the figures, tables, algorithms, or equations that carry the paper's argument, not a prose summary.",
        "",
        "## paper.md",
        "",
        f"Path: `{paper_path}`",
