...
def meridian_agents_contract_block() -> str:
    return "\n".join(
        [
            ...
            "",
            "Keep the Meridian App current even when the Lab skill is not loaded:",
            "",
            "- When discussion produces a new research direction, hypothesis, or mechanism worth revisiting,",
            "  record it with `meridian.workspace_idea_add` (CLI: `meridian workspace idea-add`). Research",
            "  ideas belong in Meridian, not in agent-local notes such as `.arbor/ideas.md`.",
            "- When the user asks to work on a Lab node, make it the active path before the first step, and",
            "  record the node's result with `meridian.lab_result` when its work ends.",
            "",
