...
## Feedback Repair Round 2

### Feedback

The user replayed the setup check and it still reported Meridian as ready while
not creating `.meridian/` in the target repo. The answer treated Lab as a next
step instead of making the local Lab research space part of setup readiness.

### Root Cause

The v0.3.6 repair still said missing `.meridian/` should be reported as
`needs_lab_init` and created only after asking. In practice, that kept the
behavior optional: an agent could finish the `meridian` status path as ready and
defer the actual skeleton creation to `lab`.

### Repair

- Hardened the `meridian` setup skill so a requested Lab-ready status check must
  create the minimal `.meridian/` skeleton in the target repo when it is
  missing.
- Clarified that this is non-destructive setup initialization, not a Lab
  workflow, and must not create threads, experiments, proposals, active nodes,
  or run research work.
- Added behavior regression coverage so future prompt edits catch the specific
  failure mode: reporting overall ready while `.meridian/` is still missing or
  deferring setup to `lab`.
...
- Full unit suite passed:
  `PYTHONPATH=src python3 -m unittest discover -s tests`
- Compile check passed:
  `PYTHONPYCACHEPREFIX=/private/tmp/meridian-setup-lab-init-pycache PYTHONPATH=src python3 -m compileall src tests`
- Diff hygiene passed:
  `git diff --check`
- Active Paper Wiki gates passed on `/Users/shawn/Desktop/paper-wiki/wiki`:
  lint, source-audit, and catalog.
- Lab initialization smoke passed: the core helper wrote only
  `.meridian/state.md`, `.meridian/memory.md`, and the three index files, then
  `validate_lab_space` returned `pass`.
- Arbor process-state returned warning-only with 0 errors; residual warnings are
  older F1/F2/F37/F38 metadata/release-round warnings.
- AGENTS project-map drift hook passed after removing a transient repo-local
  `wiki/` directory generated by the obsolete `--wiki-root wiki` gate.

## Release Round

### Decision

Release the setup-status repair as Meridian `0.3.7`. This is a patch release:
it changes the shipped `meridian` setup skill behavior, synchronized plugin
skill copies, version surfaces, and behavior regression assets. It does not add
new CLI, MCP, daemon, database, or Lab workflow surfaces.

### Release Evidence

- Version surfaces aligned to `0.3.7`: `VERSION`, `pyproject.toml`,
  `src/meridian/__init__.py`, Codex plugin manifest, Claude Code plugin
  manifest, and release-version unit test expectation.
- Regression scope covers the reported failure: setup/status can no longer
  report overall ready while requested Lab `.meridian/` skeleton is missing or
  defer skeleton creation to `lab`.
- Final release gates are rerun after version bump before commit/tag/push.
