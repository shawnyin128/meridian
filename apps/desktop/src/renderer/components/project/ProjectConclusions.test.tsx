...
const PENDING: ProjectConclusion = {
  id: 't.wide', node: 't.wide', text: 'Wide trees win at B8', date: '2026-09-20', state: 'pending',
...
describe('project conclusions', () => {
  ...
  const handlers = {
    onClose: vi.fn(), onVerify: vi.fn(), onOpenNode: vi.fn(), onOpenTask: vi.fn(), onOpenPage: vi.fn(),
  ...
  it('一行与动态同一套列:状态、日期、节点、结论、Wiki 状态;点一下打开', () => {
    const onOpen = vi.fn()
    const written = { ...PENDING, state: 'verified' as const, wiki: [{ page: 'topics/sd', title: 'Speculative decoding', claim: 'wide-b8', version: 2 }] }
    act(() => root!.render(
      <MessagesProvider>
    ...
    expect(verified!.classList.contains('selected')).toBe(true)
    act(() => pending!.click())
  ...
  it('待验证:能验证、不能写入 Wiki;证据链依次是任务、节点、实验记录,点任务和节点各自跳过去', async () => {
    await panel(PENDING)
    act(() => button('验证').click())
    expect(handlers.onVerify).toHaveBeenCalled()
  ...
  it('已验证:选一页写入 Wiki,提交一条以这个节点为证据的结论', async () => {
    await panel({ ...PENDING, state: 'verified' })
