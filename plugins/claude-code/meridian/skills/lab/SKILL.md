...
When `.meridian/control/plan.json` exists and its schema is
`meridian.project-plan.v1`, read it before choosing or executing research work.
It is the Meridian App's project-control surface: project focus, dates, tasks,
two-hour task windows, and milestones. Treat it as user-authored planning state.
The App may reach this repository locally or through SSH; the transport does
not change the file contract or Lab's ownership rules.
...
- If Lab state conflicts with the plan, report the mismatch; do not silently
  choose one or rewrite either side.
- Keep detailed experiment evidence in `.meridian/experiments/` and approach
  state in `.meridian/threads/` as before.
- Repository-side event projections may be materialized at
  `.meridian/events/events.json` using `meridian.workspace-events.v1`. Every
  event must name its durable `source` record; the event view is not evidence.
...
The Meridian App treats both the research graph and projected research events
as passive, read-only visibility into Agent work. The App remains the writer
for project tasks and milestones, but it must not create, rename, reclassify,
or delete research nodes, append node events, or attach node-level Wiki
...
experiment, optionally updates node state or next action, and creates the
compact App-visible event without duplicating the evidence into the event
projection. Pass `experiment` (title, validity, question, command, result,
interpretation) to write a new `.meridian/experiments/<id>.md` record in the
same call; omit it to attach a record that already exists.
...
- **Relations.** When work shows that two existing nodes block, support,
  contradict, or supersede each other, add a `create_edge` change; it needs no
  confirmation.
...
- On confirmation, run the deterministic Meridian helper:

  ```bash
  python -m meridian setup init-lab --lab-root <repo>
  ```

  This creates or refreshes the minimal Lab skeleton:
  - `.meridian/state.md`
  - `.meridian/threads/index.md`
  - `.meridian/experiments/index.md`
  - `.meridian/proposals/index.md`
- The same helper injects or refreshes the guarded Meridian research-agent contract block
  in project `AGENTS.md`; preserve user text outside that managed block.
- Do not hand-write, summarize, paraphrase, or otherwise generate the
  `AGENTS.md` block yourself. The block must come from the deterministic
  Meridian helper so repeated Lab initializations produce the same managed text.
