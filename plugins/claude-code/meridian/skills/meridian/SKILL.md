...
Meridian has four user-facing skills:
...
- `meridian-coding`: incremental App/Lab context before project-connected code work.
...
Use `meridian-coding` when normal code work begins in a connected repository so
the agent reads the change cursor before expanding only relevant project, idea,
or node context.
...
- Check that the plugin package exposes `meridian`, `wiki`, `lab`, and `meridian-coding` skills.
