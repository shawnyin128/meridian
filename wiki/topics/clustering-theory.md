---
type: "topic"
title: "clustering theory"
status: "active"
created: "2026-05-20"
updated: "2026-05-20"
aliases:
sources:
  - "papers/7194-FlexHiNM-GP-Flexible-Hier.md"
  - "papers/Dong-et-al-2025-AuroRA-Breaking-Low-Rank-Bottleneck-of-LoRA-with-Nonlinear-Mapping.md"
  - "papers/Ding-and-He-2004-K-means-clustering-via-principal-component-analysis.md"
  - "papers/Bauckhage-2015-k-Means-Clustering-Is-Matrix-Factorization.md"
  - "papers/Hong-et-al-2025-A-Geometric-Approach-to-k-means.md"
  - "papers/Dicks-and-Wales-2022-Elucidating-the-solution-structure-of-the-K-means-cost-function-using-energy-landscape-theory.md"
  - "papers/Tarpey-2007-A-parametric-k-means-algorithm.md"
  - "papers/Roy-2024-An-Approach-Towards-Learning-K-means-friendly-Deep-Latent-Representation.md"
  - "papers/Gao-et-al-2019-Deep-clustering-with-concrete-k-means.md"
  - "papers/Yang-et-al-Towards-K-means-friendly-Spaces-Simultaneous-Deep-Learning-and-Clustering.md"
  - "papers/Assran-et-al-2023-Self-Supervised-Learning-from-Images-with-a-Joint-Embedding-Predictive-Architecture.md"
  - "papers/Kraabel-et-al-2025-StefaLand-An-Efficient-Geoscience-Foundation-Model-That-Improves-Dynamic-Land-Surface-Predictions.md"
  - "papers/Livny-et-al-Automatic-Reconstruction-of-Tree-Skeletal-Structures-from-Point-Clouds.md"
  - "papers/Tagliasacchi-et-al-Curve-Skeleton-Extraction-from-Incomplete-Point-Cloud.md"
  - "papers/Au-et-al-2008-Skeleton-extraction-by-mesh-contraction.md"
  - "papers/Ainslie-et-al-2020-ETC-Encoding-Long-and-Structured-Inputs-in-Transformers.md"
  - "papers/Roy-et-al-2020-Efficient-Content-Based-Sparse-Attention-with-Routing-Transformers.md"
  - "papers/Zaheer-et-al-2021-Big-Bird-Transformers-for-Longer-Sequences.md"
  - "papers/Xiong-et-al-2021-Nystr-mformer-A-Nystr-m-Based-Algorithm-for-Approximating-Self-Attention.md"
  - "papers/Choromanski-et-al-2022-Rethinking-Attention-with-Performers.md"
  - "papers/Trippe-et-al-2023-Diffusion-probabilistic-modeling-of-protein-backbones-in-3D-for-the-motif-scaffolding-problem.md"
  - "papers/Bo-et-al-2020-Structural-Deep-Clustering-Network.md"
  - "papers/Ren-et-al-2025-Deep-Clustering-A-Comprehensive-Survey.md"
  - "papers/Son-et-al-2025-NSNQuant-A-Double-Normalization-Approach-for-Calibration-Free-Low-Bit-Vector-Quantization-of-KV-Cac.md"
source_papers:
  - "papers/7194-FlexHiNM-GP-Flexible-Hier.md"
  - "papers/Dong-et-al-2025-AuroRA-Breaking-Low-Rank-Bottleneck-of-LoRA-with-Nonlinear-Mapping.md"
  - "papers/Ding-and-He-2004-K-means-clustering-via-principal-component-analysis.md"
  - "papers/Bauckhage-2015-k-Means-Clustering-Is-Matrix-Factorization.md"
  - "papers/Hong-et-al-2025-A-Geometric-Approach-to-k-means.md"
  - "papers/Dicks-and-Wales-2022-Elucidating-the-solution-structure-of-the-K-means-cost-function-using-energy-landscape-theory.md"
  - "papers/Tarpey-2007-A-parametric-k-means-algorithm.md"
  - "papers/Roy-2024-An-Approach-Towards-Learning-K-means-friendly-Deep-Latent-Representation.md"
  - "papers/Gao-et-al-2019-Deep-clustering-with-concrete-k-means.md"
  - "papers/Yang-et-al-Towards-K-means-friendly-Spaces-Simultaneous-Deep-Learning-and-Clustering.md"
  - "papers/Assran-et-al-2023-Self-Supervised-Learning-from-Images-with-a-Joint-Embedding-Predictive-Architecture.md"
  - "papers/Kraabel-et-al-2025-StefaLand-An-Efficient-Geoscience-Foundation-Model-That-Improves-Dynamic-Land-Surface-Predictions.md"
  - "papers/Livny-et-al-Automatic-Reconstruction-of-Tree-Skeletal-Structures-from-Point-Clouds.md"
  - "papers/Tagliasacchi-et-al-Curve-Skeleton-Extraction-from-Incomplete-Point-Cloud.md"
  - "papers/Au-et-al-2008-Skeleton-extraction-by-mesh-contraction.md"
  - "papers/Ainslie-et-al-2020-ETC-Encoding-Long-and-Structured-Inputs-in-Transformers.md"
  - "papers/Roy-et-al-2020-Efficient-Content-Based-Sparse-Attention-with-Routing-Transformers.md"
  - "papers/Zaheer-et-al-2021-Big-Bird-Transformers-for-Longer-Sequences.md"
  - "papers/Xiong-et-al-2021-Nystr-mformer-A-Nystr-m-Based-Algorithm-for-Approximating-Self-Attention.md"
  - "papers/Choromanski-et-al-2022-Rethinking-Attention-with-Performers.md"
  - "papers/Trippe-et-al-2023-Diffusion-probabilistic-modeling-of-protein-backbones-in-3D-for-the-motif-scaffolding-problem.md"
  - "papers/Bo-et-al-2020-Structural-Deep-Clustering-Network.md"
  - "papers/Ren-et-al-2025-Deep-Clustering-A-Comprehensive-Survey.md"
  - "papers/Son-et-al-2025-NSNQuant-A-Double-Normalization-Approach-for-Calibration-Free-Low-Bit-Vector-Quantization-of-KV-Cac.md"
related_papers:
  - "papers/7194-FlexHiNM-GP-Flexible-Hier.md"
  - "papers/Dong-et-al-2025-AuroRA-Breaking-Low-Rank-Bottleneck-of-LoRA-with-Nonlinear-Mapping.md"
  - "papers/Ding-and-He-2004-K-means-clustering-via-principal-component-analysis.md"
  - "papers/Bauckhage-2015-k-Means-Clustering-Is-Matrix-Factorization.md"
  - "papers/Hong-et-al-2025-A-Geometric-Approach-to-k-means.md"
  - "papers/Dicks-and-Wales-2022-Elucidating-the-solution-structure-of-the-K-means-cost-function-using-energy-landscape-theory.md"
  - "papers/Tarpey-2007-A-parametric-k-means-algorithm.md"
  - "papers/Roy-2024-An-Approach-Towards-Learning-K-means-friendly-Deep-Latent-Representation.md"
  - "papers/Gao-et-al-2019-Deep-clustering-with-concrete-k-means.md"
  - "papers/Yang-et-al-Towards-K-means-friendly-Spaces-Simultaneous-Deep-Learning-and-Clustering.md"
  - "papers/Assran-et-al-2023-Self-Supervised-Learning-from-Images-with-a-Joint-Embedding-Predictive-Architecture.md"
  - "papers/Kraabel-et-al-2025-StefaLand-An-Efficient-Geoscience-Foundation-Model-That-Improves-Dynamic-Land-Surface-Predictions.md"
  - "papers/Livny-et-al-Automatic-Reconstruction-of-Tree-Skeletal-Structures-from-Point-Clouds.md"
  - "papers/Tagliasacchi-et-al-Curve-Skeleton-Extraction-from-Incomplete-Point-Cloud.md"
  - "papers/Au-et-al-2008-Skeleton-extraction-by-mesh-contraction.md"
  - "papers/Ainslie-et-al-2020-ETC-Encoding-Long-and-Structured-Inputs-in-Transformers.md"
  - "papers/Roy-et-al-2020-Efficient-Content-Based-Sparse-Attention-with-Routing-Transformers.md"
  - "papers/Zaheer-et-al-2021-Big-Bird-Transformers-for-Longer-Sequences.md"
  - "papers/Xiong-et-al-2021-Nystr-mformer-A-Nystr-m-Based-Algorithm-for-Approximating-Self-Attention.md"
  - "papers/Choromanski-et-al-2022-Rethinking-Attention-with-Performers.md"
  - "papers/Trippe-et-al-2023-Diffusion-probabilistic-modeling-of-protein-backbones-in-3D-for-the-motif-scaffolding-problem.md"
  - "papers/Bo-et-al-2020-Structural-Deep-Clustering-Network.md"
  - "papers/Ren-et-al-2025-Deep-Clustering-A-Comprehensive-Survey.md"
  - "papers/Son-et-al-2025-NSNQuant-A-Double-Normalization-Approach-for-Calibration-Free-Low-Bit-Vector-Quantization-of-KV-Cac.md"
related_methods:
  - "clustering algorithm"
related_topics:
supports:
contradicts:
supersedes:
superseded_by:
confidence: "medium"
review_state: "auto_structured"
evolution_state: "active"
revision_id: "knowledge-656b098e9b"
---
# clustering theory

## Scope

This topic page compiles canonical paper pages around `clustering theory`. It is a navigation and synthesis surface; paper-specific source facts remain in the linked pages.

## Key Papers

- [[papers/7194-FlexHiNM-GP-Flexible-Hier]]
- [[papers/Dong-et-al-2025-AuroRA-Breaking-Low-Rank-Bottleneck-of-LoRA-with-Nonlinear-Mapping]]
- [[papers/Ding-and-He-2004-K-means-clustering-via-principal-component-analysis]]
- [[papers/Bauckhage-2015-k-Means-Clustering-Is-Matrix-Factorization]]
- [[papers/Hong-et-al-2025-A-Geometric-Approach-to-k-means]]
- [[papers/Dicks-and-Wales-2022-Elucidating-the-solution-structure-of-the-K-means-cost-function-using-energy-landscape-theory]]
- [[papers/Tarpey-2007-A-parametric-k-means-algorithm]]
- [[papers/Roy-2024-An-Approach-Towards-Learning-K-means-friendly-Deep-Latent-Representation]]
- [[papers/Gao-et-al-2019-Deep-clustering-with-concrete-k-means]]
- [[papers/Yang-et-al-Towards-K-means-friendly-Spaces-Simultaneous-Deep-Learning-and-Clustering]]
- [[papers/Assran-et-al-2023-Self-Supervised-Learning-from-Images-with-a-Joint-Embedding-Predictive-Architecture]]
- [[papers/Kraabel-et-al-2025-StefaLand-An-Efficient-Geoscience-Foundation-Model-That-Improves-Dynamic-Land-Surface-Predictions]]
- [[papers/Livny-et-al-Automatic-Reconstruction-of-Tree-Skeletal-Structures-from-Point-Clouds]]
- [[papers/Tagliasacchi-et-al-Curve-Skeleton-Extraction-from-Incomplete-Point-Cloud]]
- [[papers/Au-et-al-2008-Skeleton-extraction-by-mesh-contraction]]
- [[papers/Ainslie-et-al-2020-ETC-Encoding-Long-and-Structured-Inputs-in-Transformers]]
- [[papers/Roy-et-al-2020-Efficient-Content-Based-Sparse-Attention-with-Routing-Transformers]]
- [[papers/Zaheer-et-al-2021-Big-Bird-Transformers-for-Longer-Sequences]]
- [[papers/Xiong-et-al-2021-Nystr-mformer-A-Nystr-m-Based-Algorithm-for-Approximating-Self-Attention]]
- [[papers/Choromanski-et-al-2022-Rethinking-Attention-with-Performers]]
- [[papers/Trippe-et-al-2023-Diffusion-probabilistic-modeling-of-protein-backbones-in-3D-for-the-motif-scaffolding-problem]]
- [[papers/Bo-et-al-2020-Structural-Deep-Clustering-Network]]
- [[papers/Ren-et-al-2025-Deep-Clustering-A-Comprehensive-Survey]]
- [[papers/Son-et-al-2025-NSNQuant-A-Double-Normalization-Approach-for-Calibration-Free-Low-Bit-Vector-Quantization-of-KV-Cac]]

## Method Families

- [[methods/clustering-algorithm|clustering algorithm]]

## Claims

- [[papers/7194-FlexHiNM-GP-Flexible-Hier|7194_FlexHiNM_GP_Flexible_Hier]]: 7194_FlexHiNM_GP_Flexible_Hier is a clustering method/theory paper: it studies how data vectors are assigned to centroids and which objective or initialization assumptions make the PCA or representation-learning connecti...
- [[papers/Dong-et-al-2025-AuroRA-Breaking-Low-Rank-Bottleneck-of-LoRA-with-Nonlinear-Mapping|Dong et al. - 2025 - AuroRA Breaking Low-Rank Bottleneck of LoRA with Nonlinear Mapping]]: Dong et al. - 2025 - AuroRA Breaking Low-Rank Bottleneck of LoRA with Nonlinear Mapping: Analyzes when clustering assignments and centroids align with PCA-style structure and which objective assumptions make that connect...
- [[papers/Ding-and-He-2004-K-means-clustering-via-principal-component-analysis|Ding and He - 2004 - K -means clustering via principal component analysis]]: Ding and He - 2004 - K -means clustering via principal component analysis is a clustering method/theory paper: it studies how data vectors are assigned to centroids and which objective or initialization assumptions make...
- [[papers/Bauckhage-2015-k-Means-Clustering-Is-Matrix-Factorization|Bauckhage - 2015 - k-Means Clustering Is Matrix Factorization]]: Bauckhage - 2015 - k-Means Clustering Is Matrix Factorization is a clustering method/theory paper: it studies how data vectors are assigned to centroids and which objective or initialization assumptions make the PCA or r...
- [[papers/Hong-et-al-2025-A-Geometric-Approach-to-k-means|Hong et al. - 2025 - A Geometric Approach to $k$-means]]: Hong et al. - 2025 - A Geometric Approach to $k$-means is a clustering method/theory paper: it studies how data vectors are assigned to centroids and which objective or initialization assumptions make the PCA or represen...
- [[papers/Dicks-and-Wales-2022-Elucidating-the-solution-structure-of-the-K-means-cost-function-using-energy-landscape-theory|Dicks and Wales - 2022 - Elucidating the solution structure of the K-means cost function using energy landscape theory]]: Dicks and Wales - 2022 - Elucidating the solution structure of the K-means cost function using energy landscape theory is a clustering method/theory paper: it studies how data vectors are assigned to centroids and which...
- [[papers/Tarpey-2007-A-parametric-k-means-algorithm|Tarpey - 2007 - A parametric k-means algorithm]]: Tarpey - 2007 - A parametric k-means algorithm is a clustering method/theory paper: it studies how data vectors are assigned to centroids and which objective or initialization assumptions make the PCA or representation-l...
- [[papers/Roy-2024-An-Approach-Towards-Learning-K-means-friendly-Deep-Latent-Representation|Roy - 2024 - An Approach Towards Learning K-means-friendly Deep Latent Representation]]: Roy - 2024 - An Approach Towards Learning K-means-friendly Deep Latent Representation is a clustering method/theory paper: it studies how data vectors are assigned to centroids and which objective or initialization assum...

## Contradictions

- No contradiction has been asserted automatically. Use knowledge repair proposals or refinement before publishing one.

## Retrieval Hooks

- Retrieve this topic when comparing papers, mechanisms, limitations, or implementation hooks around `clustering theory`.
