---
type: "topic"
title: "conditional diffusion"
status: "active"
created: "2026-05-20"
updated: "2026-05-20"
aliases:
sources:
  - "papers/Bai-et-al-2025-Whole-Body-Conditioned-Egocentric-Video-Prediction.md"
  - "papers/Ho-and-Salimans-2022-Classifier-Free-Diffusion-Guidance.md"
  - "papers/Blattmann-et-al-2023-Stable-Video-Diffusion-Scaling-Latent-Video-Diffusion-Models-to-Large-Datasets.md"
  - "papers/Srivastava-et-al-Precipitation-Downscaling-with-Spatiotemporal-Video-Diffusion.md"
  - "papers/Mardani-et-al-2024-Residual-Corrective-Diffusion-Modeling-for-Km-scale-Atmospheric-Downscaling.md"
  - "papers/Xie-et-al-2022-Crystal-Diffusion-Variational-Autoencoder-for-Periodic-Material-Generation.md"
  - "papers/Schneuing-et-al-2024-Structure-based-Drug-Design-with-Equivariant-Diffusion-Models.md"
  - "papers/Wolleb-et-al-2022-Diffusion-Models-for-Medical-Anomaly-Detection.md"
  - "papers/Dorjsembe-et-al-2024-Conditional-Diffusion-Models-for-Semantic-3D-Brain-MRI-Synthesis.md"
  - "papers/Watson-et-al-2023-De-novo-design-of-protein-structure-and-function-with-RFdiffusion.md"
  - "papers/Meng-et-al-2022-SDEdit-Guided-Image-Synthesis-and-Editing-with-Stochastic-Differential-Equations.md"
  - "papers/Yang-et-al-2025-Diffusion-Models-A-Comprehensive-Survey-of-Methods-and-Applications.md"
  - "papers/Yin-et-al-2024-A-Survey-on-Multimodal-Large-Language-Models.md"
source_papers:
  - "papers/Bai-et-al-2025-Whole-Body-Conditioned-Egocentric-Video-Prediction.md"
  - "papers/Ho-and-Salimans-2022-Classifier-Free-Diffusion-Guidance.md"
  - "papers/Blattmann-et-al-2023-Stable-Video-Diffusion-Scaling-Latent-Video-Diffusion-Models-to-Large-Datasets.md"
  - "papers/Srivastava-et-al-Precipitation-Downscaling-with-Spatiotemporal-Video-Diffusion.md"
  - "papers/Mardani-et-al-2024-Residual-Corrective-Diffusion-Modeling-for-Km-scale-Atmospheric-Downscaling.md"
  - "papers/Xie-et-al-2022-Crystal-Diffusion-Variational-Autoencoder-for-Periodic-Material-Generation.md"
  - "papers/Schneuing-et-al-2024-Structure-based-Drug-Design-with-Equivariant-Diffusion-Models.md"
  - "papers/Wolleb-et-al-2022-Diffusion-Models-for-Medical-Anomaly-Detection.md"
  - "papers/Dorjsembe-et-al-2024-Conditional-Diffusion-Models-for-Semantic-3D-Brain-MRI-Synthesis.md"
  - "papers/Watson-et-al-2023-De-novo-design-of-protein-structure-and-function-with-RFdiffusion.md"
  - "papers/Meng-et-al-2022-SDEdit-Guided-Image-Synthesis-and-Editing-with-Stochastic-Differential-Equations.md"
  - "papers/Yang-et-al-2025-Diffusion-Models-A-Comprehensive-Survey-of-Methods-and-Applications.md"
  - "papers/Yin-et-al-2024-A-Survey-on-Multimodal-Large-Language-Models.md"
related_papers:
  - "papers/Bai-et-al-2025-Whole-Body-Conditioned-Egocentric-Video-Prediction.md"
  - "papers/Ho-and-Salimans-2022-Classifier-Free-Diffusion-Guidance.md"
  - "papers/Blattmann-et-al-2023-Stable-Video-Diffusion-Scaling-Latent-Video-Diffusion-Models-to-Large-Datasets.md"
  - "papers/Srivastava-et-al-Precipitation-Downscaling-with-Spatiotemporal-Video-Diffusion.md"
  - "papers/Mardani-et-al-2024-Residual-Corrective-Diffusion-Modeling-for-Km-scale-Atmospheric-Downscaling.md"
  - "papers/Xie-et-al-2022-Crystal-Diffusion-Variational-Autoencoder-for-Periodic-Material-Generation.md"
  - "papers/Schneuing-et-al-2024-Structure-based-Drug-Design-with-Equivariant-Diffusion-Models.md"
  - "papers/Wolleb-et-al-2022-Diffusion-Models-for-Medical-Anomaly-Detection.md"
  - "papers/Dorjsembe-et-al-2024-Conditional-Diffusion-Models-for-Semantic-3D-Brain-MRI-Synthesis.md"
  - "papers/Watson-et-al-2023-De-novo-design-of-protein-structure-and-function-with-RFdiffusion.md"
  - "papers/Meng-et-al-2022-SDEdit-Guided-Image-Synthesis-and-Editing-with-Stochastic-Differential-Equations.md"
  - "papers/Yang-et-al-2025-Diffusion-Models-A-Comprehensive-Survey-of-Methods-and-Applications.md"
  - "papers/Yin-et-al-2024-A-Survey-on-Multimodal-Large-Language-Models.md"
related_methods:
  - "conditional diffusion"
  - "transformer architecture"
  - "preference-based reinforcement learning"
  - "reward modeling"
  - "paper-specific research method"
  - "semantic image synthesis"
  - "survey synthesis"
related_topics:
supports:
contradicts:
supersedes:
superseded_by:
confidence: "medium"
review_state: "auto_structured"
evolution_state: "active"
revision_id: "knowledge-eb62b7d9da"
---
# conditional diffusion

## Scope

This topic page compiles canonical paper pages around `conditional diffusion`. It is a navigation and synthesis surface; paper-specific source facts remain in the linked pages.

## Key Papers

- [[papers/Bai-et-al-2025-Whole-Body-Conditioned-Egocentric-Video-Prediction]]
- [[papers/Ho-and-Salimans-2022-Classifier-Free-Diffusion-Guidance]]
- [[papers/Blattmann-et-al-2023-Stable-Video-Diffusion-Scaling-Latent-Video-Diffusion-Models-to-Large-Datasets]]
- [[papers/Srivastava-et-al-Precipitation-Downscaling-with-Spatiotemporal-Video-Diffusion]]
- [[papers/Mardani-et-al-2024-Residual-Corrective-Diffusion-Modeling-for-Km-scale-Atmospheric-Downscaling]]
- [[papers/Xie-et-al-2022-Crystal-Diffusion-Variational-Autoencoder-for-Periodic-Material-Generation]]
- [[papers/Schneuing-et-al-2024-Structure-based-Drug-Design-with-Equivariant-Diffusion-Models]]
- [[papers/Wolleb-et-al-2022-Diffusion-Models-for-Medical-Anomaly-Detection]]
- [[papers/Dorjsembe-et-al-2024-Conditional-Diffusion-Models-for-Semantic-3D-Brain-MRI-Synthesis]]
- [[papers/Watson-et-al-2023-De-novo-design-of-protein-structure-and-function-with-RFdiffusion]]
- [[papers/Meng-et-al-2022-SDEdit-Guided-Image-Synthesis-and-Editing-with-Stochastic-Differential-Equations]]
- [[papers/Yang-et-al-2025-Diffusion-Models-A-Comprehensive-Survey-of-Methods-and-Applications]]
- [[papers/Yin-et-al-2024-A-Survey-on-Multimodal-Large-Language-Models]]

## Method Families

- [[methods/conditional-diffusion|conditional diffusion]]
- [[methods/transformer-architecture|transformer architecture]]
- [[methods/preference-based-reinforcement-learning|preference-based reinforcement learning]]
- [[methods/reward-modeling|reward modeling]]
- [[methods/paper-specific-research-method|paper-specific research method]]
- [[methods/semantic-image-synthesis|semantic image synthesis]]
- [[methods/survey-synthesis|survey synthesis]]

## Claims

- [[papers/Bai-et-al-2025-Whole-Body-Conditioned-Egocentric-Video-Prediction|Bai et al. - 2025 - Whole-Body Conditioned Egocentric Video Prediction]]: Bai et al. - 2025 - Whole-Body Conditioned Egocentric Video Prediction: We train an auto-regressive conditional diffusion transformer on Nymeria, a large-scale dataset of real-world egocentric video and body pose capture...
- [[papers/Ho-and-Salimans-2022-Classifier-Free-Diffusion-Guidance|Ho and Salimans - 2022 - Classifier-Free Diffusion Guidance]]: Ho and Salimans - 2022 - Classifier-Free Diffusion Guidance: 1 INTRODUCTION Diffusion models have recently emerged as an expressive and ﬂexible family of generative models, delivering competitive sample quality and likel...
- [[papers/Blattmann-et-al-2023-Stable-Video-Diffusion-Scaling-Latent-Video-Diffusion-Models-to-Large-Datasets|Blattmann et al. - 2023 - Stable Video Diffusion Scaling Latent Video Diffusion Models to Large Datasets]]: Blattmann et al. - 2023 - Stable Video Diffusion Scaling Latent Video Diffusion Models to Large Datasets turns preference data into a policy-optimization objective. The reusable contract is preference pairs and a referen...
- [[papers/Srivastava-et-al-Precipitation-Downscaling-with-Spatiotemporal-Video-Diffusion|Srivastava et al. - Precipitation Downscaling with Spatiotemporal Video Diffusion]]: Srivastava et al. - Precipitation Downscaling with Spatiotemporal Video Diffusion: It deterministically downscales a low- resolution precipitation sequence using spatio-temporal factorized attention and models residuals...
- [[papers/Mardani-et-al-2024-Residual-Corrective-Diffusion-Modeling-for-Km-scale-Atmospheric-Downscaling|Mardani et al. - 2024 - Residual Corrective Diffusion Modeling for Km-scale Atmospheric Downscaling]]: Mardani et al. - 2024 - Residual Corrective Diffusion Modeling for Km-scale Atmospheric Downscaling: Residual Corrective Diffusion Modeling for Km-scale Atmospheric Downscaling Morteza Mardani1,*,a, Noah Brenowitz1,*, Ya...
- [[papers/Xie-et-al-2022-Crystal-Diffusion-Variational-Autoencoder-for-Periodic-Material-Generation|Xie et al. - 2022 - Crystal Diffusion Variational Autoencoder for Periodic Material Generation]]: Xie et al. - 2022 - Crystal Diffusion Variational Autoencoder for Periodic Material Generation: We propose a Crystal Diffusion Variational Autoencoder (CDVAE) that captures the physical inductive bias of material stabili...
- [[papers/Schneuing-et-al-2024-Structure-based-Drug-Design-with-Equivariant-Diffusion-Models|Schneuing et al. - 2024 - Structure-based Drug Design with Equivariant Diffusion Models]]: Schneuing et al. - 2024 - Structure-based Drug Design with Equivariant Diffusion Models: In this study, we propose DiffSBDD, an SE(3)-equivariant 3D-conditional diffusion model for SBDD that respects translation, rotatio...
- [[papers/Wolleb-et-al-2022-Diffusion-Models-for-Medical-Anomaly-Detection|Wolleb et al. - 2022 - Diffusion Models for Medical Anomaly Detection]]: Wolleb et al. - 2022 - Diffusion Models for Medical Anomaly Detection: In Algorithm 1, we present the workﬂow of our approach. We present a novel weakly supervised anomaly de- tection method based on denoising diﬀusion i...

## Contradictions

- No contradiction has been asserted automatically. Use knowledge repair proposals or refinement before publishing one.

## Retrieval Hooks

- Retrieve this topic when comparing papers, mechanisms, limitations, or implementation hooks around `conditional diffusion`.
## Key Concepts

- [[concepts/Diffusion-conditioning-signal|Diffusion conditioning signal]]
