---
type: "topic"
title: "model training dynamics"
status: "active"
created: "2026-05-20"
updated: "2026-05-20"
aliases:
sources:
  - "papers/Wei-et-al-2025-First-SFT-Second-RL-Third-UPT-Continual-Improving-Multi-Modal-LLM-Reasoning-via-Unsupervised-Post.md"
  - "papers/Qi-et-al-2025-EvoLM-In-Search-of-Lost-Language-Model-Training-Dynamics.md"
  - "papers/Michel-et-al-2019-Are-Sixteen-Heads-Really-Better-than-One.md"
  - "papers/Dong-et-al-2024-Self-Boosting-Large-Language-Models-with-Synthetic-Preference-Data.md"
  - "papers/Zuo-et-al-2025-TTRL-Test-Time-Reinforcement-Learning.md"
  - "papers/Ding-and-Zhang-2025-Sherlock-Self-Correcting-Reasoning-in-Vision-Language-Models.md"
  - "papers/Yuan-et-al-2025-Self-Rewarding-Language-Models.md"
  - "papers/Chen-et-al-2026-VL-JEPA-Joint-Embedding-Predictive-Architecture-for-Vision-language.md"
  - "papers/Xu-et-al-2025-Speculative-Knowledge-Distillation-Bridging-the-Teacher-Student-Gap-Through-Interleaved-Sampling.md"
  - "papers/DeepSeek-AI-2025-DeepSeek-R1-Incentivizing-Reasoning-Capability-in-LLMs-via-Reinforcement-Learning.md"
  - "papers/DeepSeek-AI-2024-DeepSeek-V2-A-Strong-Economical-and-Efficient-Mixture-of-Experts-Language-Model.md"
  - "papers/Dai-2024-DeepSeekMoE-Towards-Ultimate-Expert-Specialization-in-Mixture-of-Experts-Language-Models.md"
source_papers:
  - "papers/Wei-et-al-2025-First-SFT-Second-RL-Third-UPT-Continual-Improving-Multi-Modal-LLM-Reasoning-via-Unsupervised-Post.md"
  - "papers/Qi-et-al-2025-EvoLM-In-Search-of-Lost-Language-Model-Training-Dynamics.md"
  - "papers/Michel-et-al-2019-Are-Sixteen-Heads-Really-Better-than-One.md"
  - "papers/Dong-et-al-2024-Self-Boosting-Large-Language-Models-with-Synthetic-Preference-Data.md"
  - "papers/Zuo-et-al-2025-TTRL-Test-Time-Reinforcement-Learning.md"
  - "papers/Ding-and-Zhang-2025-Sherlock-Self-Correcting-Reasoning-in-Vision-Language-Models.md"
  - "papers/Yuan-et-al-2025-Self-Rewarding-Language-Models.md"
  - "papers/Chen-et-al-2026-VL-JEPA-Joint-Embedding-Predictive-Architecture-for-Vision-language.md"
  - "papers/Xu-et-al-2025-Speculative-Knowledge-Distillation-Bridging-the-Teacher-Student-Gap-Through-Interleaved-Sampling.md"
  - "papers/DeepSeek-AI-2025-DeepSeek-R1-Incentivizing-Reasoning-Capability-in-LLMs-via-Reinforcement-Learning.md"
  - "papers/DeepSeek-AI-2024-DeepSeek-V2-A-Strong-Economical-and-Efficient-Mixture-of-Experts-Language-Model.md"
  - "papers/Dai-2024-DeepSeekMoE-Towards-Ultimate-Expert-Specialization-in-Mixture-of-Experts-Language-Models.md"
related_papers:
  - "papers/Wei-et-al-2025-First-SFT-Second-RL-Third-UPT-Continual-Improving-Multi-Modal-LLM-Reasoning-via-Unsupervised-Post.md"
  - "papers/Qi-et-al-2025-EvoLM-In-Search-of-Lost-Language-Model-Training-Dynamics.md"
  - "papers/Michel-et-al-2019-Are-Sixteen-Heads-Really-Better-than-One.md"
  - "papers/Dong-et-al-2024-Self-Boosting-Large-Language-Models-with-Synthetic-Preference-Data.md"
  - "papers/Zuo-et-al-2025-TTRL-Test-Time-Reinforcement-Learning.md"
  - "papers/Ding-and-Zhang-2025-Sherlock-Self-Correcting-Reasoning-in-Vision-Language-Models.md"
  - "papers/Yuan-et-al-2025-Self-Rewarding-Language-Models.md"
  - "papers/Chen-et-al-2026-VL-JEPA-Joint-Embedding-Predictive-Architecture-for-Vision-language.md"
  - "papers/Xu-et-al-2025-Speculative-Knowledge-Distillation-Bridging-the-Teacher-Student-Gap-Through-Interleaved-Sampling.md"
  - "papers/DeepSeek-AI-2025-DeepSeek-R1-Incentivizing-Reasoning-Capability-in-LLMs-via-Reinforcement-Learning.md"
  - "papers/DeepSeek-AI-2024-DeepSeek-V2-A-Strong-Economical-and-Efficient-Mixture-of-Experts-Language-Model.md"
  - "papers/Dai-2024-DeepSeekMoE-Towards-Ultimate-Expert-Specialization-in-Mixture-of-Experts-Language-Models.md"
related_methods:
  - "preference-based reinforcement learning"
  - "reward modeling"
  - "transformer architecture"
  - "test-time reinforcement learning"
  - "paper-specific research method"
  - "RLHF"
  - "self-rewarding model training"
  - "LLM-as-judge reward modeling"
  - "video representation learning"
  - "joint embedding predictive learning"
  - "speculative decoding"
  - "long-context inference"
  - "KV-cache compression"
  - "post-training quantization"
  - "MoE quantization"
  - "expert-aware quantization"
related_topics:
supports:
contradicts:
supersedes:
superseded_by:
confidence: "medium"
review_state: "auto_structured"
evolution_state: "active"
revision_id: "knowledge-503e9a21ab"
---
# model training dynamics

## Scope

This topic page compiles canonical paper pages around `model training dynamics`. It is a navigation and synthesis surface; paper-specific source facts remain in the linked pages.

## Key Papers

- [[papers/Wei-et-al-2025-First-SFT-Second-RL-Third-UPT-Continual-Improving-Multi-Modal-LLM-Reasoning-via-Unsupervised-Post]]
- [[papers/Qi-et-al-2025-EvoLM-In-Search-of-Lost-Language-Model-Training-Dynamics]]
- [[papers/Michel-et-al-2019-Are-Sixteen-Heads-Really-Better-than-One]]
- [[papers/Dong-et-al-2024-Self-Boosting-Large-Language-Models-with-Synthetic-Preference-Data]]
- [[papers/Zuo-et-al-2025-TTRL-Test-Time-Reinforcement-Learning]]
- [[papers/Ding-and-Zhang-2025-Sherlock-Self-Correcting-Reasoning-in-Vision-Language-Models]]
- [[papers/Yuan-et-al-2025-Self-Rewarding-Language-Models]]
- [[papers/Chen-et-al-2026-VL-JEPA-Joint-Embedding-Predictive-Architecture-for-Vision-language]]
- [[papers/Xu-et-al-2025-Speculative-Knowledge-Distillation-Bridging-the-Teacher-Student-Gap-Through-Interleaved-Sampling]]
- [[papers/DeepSeek-AI-2025-DeepSeek-R1-Incentivizing-Reasoning-Capability-in-LLMs-via-Reinforcement-Learning]]
- [[papers/DeepSeek-AI-2024-DeepSeek-V2-A-Strong-Economical-and-Efficient-Mixture-of-Experts-Language-Model]]
- [[papers/Dai-2024-DeepSeekMoE-Towards-Ultimate-Expert-Specialization-in-Mixture-of-Experts-Language-Models]]

## Method Families

- [[methods/preference-based-reinforcement-learning|preference-based reinforcement learning]]
- [[methods/reward-modeling|reward modeling]]
- [[methods/transformer-architecture|transformer architecture]]
- [[methods/test-time-reinforcement-learning|test-time reinforcement learning]]
- [[methods/paper-specific-research-method|paper-specific research method]]
- [[methods/RLHF|RLHF]]
- [[methods/self-rewarding-model-training|self-rewarding model training]]
- [[methods/LLM-as-judge-reward-modeling|LLM-as-judge reward modeling]]
- [[methods/video-representation-learning|video representation learning]]
- [[methods/joint-embedding-predictive-learning|joint embedding predictive learning]]
- [[methods/speculative-decoding|speculative decoding]]
- [[methods/long-context-inference|long-context inference]]
- [[methods/KV-cache-compression|KV-cache compression]]
- [[methods/post-training-quantization|post-training quantization]]
- [[methods/MoE-quantization|MoE quantization]]
- [[methods/expert-aware-quantization|expert-aware quantization]]

## Claims

- [[papers/Wei-et-al-2025-First-SFT-Second-RL-Third-UPT-Continual-Improving-Multi-Modal-LLM-Reasoning-via-Unsupervised-Post|Wei et al. - 2025 - First SFT, Second RL, Third UPT Continual Improving Multi-Modal LLM Reasoning via Unsupervised Post]]: Wei et al. - 2025 - First SFT, Second RL, Third UPT Continual Improving Multi-Modal LLM Reasoning via Unsupervised Post turns preference data into a policy-optimization objective. The reusable contract is preference pair...
- [[papers/Qi-et-al-2025-EvoLM-In-Search-of-Lost-Language-Model-Training-Dynamics|Qi et al. - 2025 - EvoLM In Search of Lost Language Model Training Dynamics]]: Qi et al. - 2025 - EvoLM In Search of Lost Language Model Training Dynamics turns preference data into a policy-optimization objective. The reusable contract is preference pairs and a reference policy -> loss/reward sign...
- [[papers/Michel-et-al-2019-Are-Sixteen-Heads-Really-Better-than-One|Michel et al. - 2019 - Are Sixteen Heads Really Better than One]]: Michel et al. - 2019 - Are Sixteen Heads Really Better than One: 2 Background: Attention, Multi-headed Attention, and Masking In this section we lay out the notational groundwork regarding attention, and also describe ou...
- [[papers/Dong-et-al-2024-Self-Boosting-Large-Language-Models-with-Synthetic-Preference-Data|Dong et al. - 2024 - Self-Boosting Large Language Models with Synthetic Preference Data]]: Dong et al. - 2024 - Self-Boosting Large Language Models with Synthetic Preference Data: Deploys clustered weights through lookup-table kernels so non-uniform centroids can be used for lower memory and faster inference....
- [[papers/Zuo-et-al-2025-TTRL-Test-Time-Reinforcement-Learning|Zuo et al. - 2025 - TTRL Test-Time Reinforcement Learning]]: Zuo et al. - 2025 - TTRL Test-Time Reinforcement Learning: Applies reinforcement-learning style updates at test time, so evaluation must separate task reward, rollout/update behavior, and inference-time cost. It operates...
- [[papers/Ding-and-Zhang-2025-Sherlock-Self-Correcting-Reasoning-in-Vision-Language-Models|Ding and Zhang - 2025 - Sherlock Self-Correcting Reasoning in Vision-Language Models]]: Ding and Zhang - 2025 - Sherlock Self-Correcting Reasoning in Vision-Language Models: Based on our findings, we introduce Sherlock, a self-correction and self-improvement training framework. To enable self-correction in...
- [[papers/Yuan-et-al-2025-Self-Rewarding-Language-Models|Yuan et al. - 2025 - Self-Rewarding Language Models]]: Yuan et al. - 2025 - Self-Rewarding Language Models turns preference data into a policy-optimization objective. The reusable contract is preference pairs and a reference policy -> loss/reward signal -> aligned policy beh...
- [[papers/Chen-et-al-2026-VL-JEPA-Joint-Embedding-Predictive-Architecture-for-Vision-language|Chen et al. - 2026 - VL-JEPA Joint Embedding Predictive Architecture for Vision-language]]: Chen et al. - 2026 - VL-JEPA Joint Embedding Predictive Architecture for Vision-language: Trains a vision-language model in embedding space rather than token reconstruction: caption pretraining aligns image/text embeddin...

## Contradictions

- No contradiction has been asserted automatically. Use knowledge repair proposals or refinement before publishing one.

## Retrieval Hooks

- Retrieve this topic when comparing papers, mechanisms, limitations, or implementation hooks around `model training dynamics`.
## Key Concepts

- [[concepts/KL-regularization|KL regularization]]
